...
temp <- terra::rast("C:/Users/gkonstan/OneDrive - Imperial College London/meteo_sri_lanka/Output/2m_dewpoint_temperature2017-04-01_2017-05-01.nc")


shp_file <- list.files(paste0("Output/"), pattern = "\\.shp$", full.names = TRUE)
level <- 1
shp_string <- shp_file[stringr::str_sub(gsub(".shp", "", shp_file), -1) == level]
shp <- sf::read_sf(shp_string)


ggplot() + 
  geom_spatraster(data = temp[[1]]) + 
  geom_sf(data = shp, fill=NA, col = "black", lwd = 0.5) + 
  scale_fill_viridis_c(na.value = NA) + theme_bw()
...
shp_0 <- read_sf("Output/gadm41_LKA_0.shp")
shp_1 <- read_sf("Output/gadm41_LKA_1.shp")
shp_2 <- read_sf("Output/gadm41_LKA_2.shp")

(ggplot() + 
  geom_sf(data = shp_0, fill = NA) + theme_bw() + 
    ggtitle("Admin 0") + 
    theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=0.5)))|
  (ggplot() + 
  geom_sf(data = shp_1, fill = NA) + theme_bw() + 
    ggtitle("Admin 1") + 
    theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=0.5)))|
  (ggplot() + 
  geom_sf(data = shp_2, fill = NA) + theme_bw() + 
    ggtitle("Admin 2") + 
    theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=0.5)))
```
...
The population is available at 100m grid, whereas the ERA-5 (for instance) at 9km. The file *05_AssignPopWeights.R*, assigns to the 9km ERA-5 (for instance) centroids the closest centroids of the 100m population grid. After finding the closest neighbors, the file sums up the population and assigns this number to the 9km grid.
...
ggplot() + 
  ...
  geom_point(data = dat_points, aes(x = x, y = y), col = "red", cex = 0.7) + 
  theme_bw()
